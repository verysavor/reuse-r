...
class BlockchainAPI:
    def __init__(self):
        ...
        self.current_api = 0
        # High-performance concurrency limits for maximum speed
        self.rate_limit_semaphore = asyncio.Semaphore(100)  # High throughput
        
        # Aggressive API semaphores for maximum speed (balanced to avoid rate limits)
        self.blockstream_semaphore = asyncio.Semaphore(25)  # Increased
        self.mempool_semaphore = asyncio.Semaphore(25)      # Increased 
        self.cryptoapis_semaphore = asyncio.Semaphore(50)   # Higher for paid API
    ...
    async def make_parallel_api_request(self, endpoint_func, *args):
        ...
        tasks = [asyncio.create_task(try_api(api_type, api_base, semaphore)) 
    ...
    async def process_single_block(self, semaphore: asyncio.Semaphore, scan_id: str, block_num: int, address_types: List[str]) -> Dict:
        ...
        async with semaphore:
            try:
                ...
                # Process transactions in parallel with maximum concurrency for speed
                tx_semaphore = asyncio.Semaphore(self.max_concurrent_transactions)
                tx_tasks = []
